...
class BuyOrderService {
  async addItemToBuyOder(data) {
    return await axios({
      method: "post",
      url: "https://localhost:5196/api/BuyOrder",
      headers: {
        accepts: "*/*",
  ...
  async getByOrderById(userId) {
    const res = await axios.get(
      `https://localhost:5196/api/BuyOrder/user/${userId}`
    );
    return res;
  }
  async getByOrderBySellerName(sellerName) {
    const res = await axios.get(
      `https://localhost:5196/api/BuyOrder/seller/${sellerName}`
    );
    return res;
  }
  async updateStatus(id) {
    try {
      const response = await axios.put(
        `https://localhost:5196/api/BuyOrder/${id}/update-status`
      );
